...
class File : base::StaticClass {
 public:
    ...
    static base::type::fstream_t* newFileStream(const std::string& filename) {
        // XXX/Yawning - obfsclient wants fresh logs per invocation
#if 0
        base::type::fstream_t *fs = new base::type::fstream_t(filename.c_str(), base::type::fstream_t::out | base::type::fstream_t::app);
#else
        base::type::fstream_t *fs = new base::type::fstream_t(filename.c_str(), base::type::fstream_t::out | base::type::fstream_t::trunc);
#endif
        if (fs->is_open()) {
            fs->flush();
        } else {
...
class TypedConfigurations : public base::threading::ThreadSafe {
 ...
 private:
    ...
    inline Conf_T unsafeGetConfigByVal(const Level& level, const std::map<Level, Conf_T>* confMap, const char* confName) {
        ...
        if (it == confMap->end()) {
            // XXX/Yawning - obfsclient oesn't use exceptions
#if 0
            try {
